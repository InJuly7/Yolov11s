...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: tuple = ()):
        ...
        self.stride = torch.tensor([8.0, 16.0, 32.0])  # yolov11s strides
...
def test_detect():
    p3 = torch.randn(1, 128, 80, 80).to(device="cuda", dtype=torch.float32)
    ...
    x = [p3, p4, p5]
    model = Detect(nc=80, ch=(128, 256, 512)).to(dtype=torch.float32, device="cuda")
    model.eval()
    with torch.no_grad():
        y, _ = model(x)
    print("Detect Output:", y.shape)
